...
module.exports = class StorageEngine {
    ...
    getAllAccounts() {
        ...
        for(let i = 0; i < accounts.length; i++) {
            let totpPin = new TOTP(accounts[i].secret);
    ...
    renameAccount(name, newName) {
        let accounts = this._decryptAccounts();
        let indexOld = this._indexInAccount(newName);
        let index = this._indexInAccount(name);
        if(indexOld === index) {
            return;
        }
        if(indexOld > -1) {
    ...
    addAccount(account) {
        let accounts = this._decryptAccounts();
        let index = this._indexInAccount(account.name);
        if (index === -1) {
